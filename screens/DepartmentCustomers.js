...
const DepartmentCustomers = ({ route, navigation }) => {
    ...
    const unsubscribe = USERS
      ...
      .onSnapshot((querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
          const { fullname, email, phone, address, avatar, department } = doc.data();
          customerList.push({
            id: doc.id,
            fullname,
            email,
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Danh sách người dùng phòng {departmentName}</Text>
      <Searchbar
      ...
      {filteredCustomers.length > 0 ? (
        ...
        <Text style={styles.noUsersText}>Không có người dùng nào trong phòng này.</Text>
      )}
    </View>
  );
};
...
export default DepartmentCustomers;
